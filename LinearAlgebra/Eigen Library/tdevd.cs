...
class tdevd
{
    ...
    Output parameters:
        ...
        Z       -   if ZNeeded is equal to:
                    ...
                    If ZNeeded<3, Z is the array whose indexes range within [1..N, 1..N].
                    In that case, the eigenvectors are stored in the matrix columns.
                    If ZNeeded=3, Z is the array whose indexes range within [1..1, 1..N].
        ...
        while( true )
        {
            ...
            if( l1<=nm1 )
            {
                for(m=l1; m<=nm1; m++)
                {
                    ...
                    if( tst<=Math.Sqrt(Math.Abs(d[m]))*Math.Sqrt(Math.Abs(d[m+1]))*eps )
                    {
                        e[m] = 0;
                        gotoflag = true;
                        break;
                ...
                while( true )
                {
                    ...
                    if( l!=lend )
                    {
                        ...
                        for(m=l; m<=lendm1; m++)
                        {
                            ...
                            if( tst<=eps2*Math.Abs(d[m])*Math.Abs(d[m+1])+safmin )
                            {
                                gotoflag = true;
                                break;
                            }
